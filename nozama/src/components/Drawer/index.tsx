import {
    Drawer,
    DrawerBody,
    DrawerFooter,
    DrawerHeader,
    DrawerOverlay,
    DrawerContent,
    DrawerCloseButton,
    Input,
    Button,
    HStack,
    VStack,
    Text,
    useNumberInput
} from '@chakra-ui/react'
import Card from '../Card'
import styles from './styles.module.scss'
import Image from 'next/image'
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome"
import { faClose, faPlus, faMinus } from '@fortawesome/free-solid-svg-icons'
import { useState } from 'react'
import useCartStore from '../../stories/cartStore'
import useUserStore from '../../stories/userStore'
import {createOrder } from '../../services/order'


export default function DrawerCart({ isOpen, onClose, cart, handleRemove }: any) {
    const [qtd, setQtd] = useState(1)
    const { minusProduct, setCart, esvaziar } = useCartStore()
    const {UserLogin } = useUserStore()

    function sumTotal() {
        if (cart.length > 0) {
            const sumCart = cart.reduce((partial: any, item: any) => partial + (item.price * item.qty), 0)
            return sumCart
        }
        else {
            return 0
        }
    }

    const completeOrder = async () =>{
        const data = {userId: UserLogin.uuid,
        products: cart }
        console.log(cart)
        await createOrder(data)
        esvaziar()
        onClose()
    }
    return (
        <Drawer
            isOpen={isOpen}
            placement='right'
            onClose={onClose}
            size={'sm'}
        >
            <DrawerOverlay />
            <DrawerContent>
                <DrawerCloseButton />
                <DrawerHeader>My Cart({cart.length})</DrawerHeader>
                <DrawerBody>
                    {/* <Input placeholder='Type here...' /> */}
                    {cart?.map((item: any, indice: number) => (
                        <HStack key={indice} className={styles.product}>
                            <HStack>
                                <Image height={80} width={80} src={item.image || "data:image/jpeg;base64,"} alt={'mouse'} />
                                <VStack className={styles.product__info}>
                                    <Text className={styles.product__info__name} >{item.name}</Text>
                                    <Text >{item.price}</Text>
                                </VStack>
                            </HStack>
                            <VStack className={styles.product__actions}>
                                <FontAwesomeIcon onClick={() => handleRemove(item.name)} icon={faClose} />
                                <HStack className={''} maxW='100px'>
                                    <FontAwesomeIcon className={styles.product__actions__icons} onClick={() => minusProduct(item)} icon={faMinus} />
                                    <Input value={item.qty} />
                                    <FontAwesomeIcon className={styles.product__actions__icons} onClick={() => setCart(item)} icon={faPlus} />
                                </HStack>
                            </VStack>
                        </HStack>
                    ))}
                </DrawerBody>
                <DrawerFooter className={styles.footer}>
                    <HStack className={styles.footer__price}>
                        <Text className={styles.footer__price__text} >SubTotal</Text>
                        <Text className={styles.footer__price__text} >${sumTotal()}</Text>
                    </HStack>
                    <Button onClick={completeOrder} className={styles.footer__button} colorScheme='blue'>Finish purchase</Button>
                </DrawerFooter>
            </DrawerContent>
        </Drawer>
    )
}
